...
typedef svg_status_t svgint_status_t;

	typedef enum svg_overflow {
...
typedef struct svg_path {
	...
	void *cache; // pointer to a cached version of the path, in an engine specific format
} svg_path_t;
...
#define SVG_STYLE_FLAG_FILTER				0x80000000000ULL
...
typedef struct svg_style {
    ...
    svg_element_t*			filter_element; /* NULL == "inherit" or pointer to filter element */
...
svg_status_t
_svg_fetch_element_by_class (svg_t *svg, const char *class_id, svg_element_t *parent, svg_element_t **element_ret);
...
svg_status_t _svg_gradient_init_copy (svg_gradient_t *gradient,
...
svg_status_t
_svg_parser_unspoof_state(svg_parser_t *parser);
...
svg_status_t _svg_path_init_copy (svg_path_t *path,
				  svg_path_t *other);

svg_status_t _svg_rect_init_copy (svg_rect_element_t *path,
...
svg_status_t _svg_pattern_init_copy (svg_pattern_t *pattern,
				     svg_pattern_t *other);
